...
throw new Error(`Unknown brand: ${brand}`);
    ...
    // Extract code from the signin redirect location
    let codeParam = null;
    try {
        ...
        codeParam = locUrl.searchParams.get('code') || null;
    } catch (e) {
        if (e instanceof Error && e.message.startsWith('OAuth error')) {
throw e;
}
        info(`[tokenManager] Step 3: cannot parse location — ${e instanceof Error ? e.message : String(e)}`);
    }

    if (!codeParam) {
        throw new Error(`No authorization code in signin redirect. Location: ${signinLocation.slice(0, 300)}`);
    }
