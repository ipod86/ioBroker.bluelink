...
throw new Error(`Unknown brand: ${brand}`);
    ...
    // If redirect is back to the IDP login page, credentials are wrong or CSRF failed
    if (step3Location.includes(host)) {
        const loc3 = new URL(step3Location, `https://${host}`);
        const err3 = loc3.searchParams.get('error') || loc3.searchParams.get('msg');
        throw new Error(`Signin failed — redirected back to IDP: ${err3 || step3Location.slice(0, 200)}`);
    }
    info(`[tokenManager] Step 3: signin accepted (redirected to: ${step3Location.split('?')[0].slice(0, 60)})`);
