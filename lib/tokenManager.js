...
nextUri += `${nextUri.includes('?') ? '&' : '?'  }country=${country}`;
    ...
    // This sends the account cookie (Domain=hyundai.com) to the CCAPI domain.
    // CCAPI recognises the IDP account cookie and marks the connector session as
    // ready (returns 204 when /session is subsequently called by the SPA JS).
    // This step activates the connector session so the IDP can issue a code after login.
    info(`[tokenManager] Step 1b: GET CCAPI SPA URL (activates connector session)`);
    try {
