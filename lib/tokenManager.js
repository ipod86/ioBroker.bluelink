...
async function fetchToken(brand, username, password, log) {
    ...
    if (step2.statusCode !== 200) throw new Error(`Certs endpoint returned ${step2.statusCode}: ${step2.body.slice(0, 200)}`);
    const jwk = JSON.parse(step2.body).retValue;
    if (!jwk || !jwk.kid) throw new Error(`No JWK in certs response: ${step2.body.slice(0, 200)}`);
    ...
    if (!codeParam) throw new Error(`No code in redirect location: ${location.slice(0, 200)}`);
    ...
    if (!tokens.refresh_token) throw new Error(`No refresh_token in token response: ${step4.body.slice(0, 200)}`);
