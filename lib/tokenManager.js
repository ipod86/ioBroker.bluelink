...
throw new Error(`Unknown brand: ${brand}`);
    ...
    // Step 1b: Follow redirect to login page and extract hidden form fields
    let connectorSessionKey = '';
    let csrfToken = '';
    const loginPageUrl = step1.headers['location'];
    if (loginPageUrl) {
        info(`[tokenManager] Step 1b: GET login page ${loginPageUrl.slice(0, 80)}`);
        const loginParsed = loginPageUrl.startsWith('http') ? new URL(loginPageUrl) : null;
        const loginPageHost = loginParsed ? loginParsed.hostname : host;
        const loginPagePort = loginParsed ? (loginParsed.port || 443) : 443;
        const loginPath = loginParsed ? (loginParsed.pathname + loginParsed.search) : loginPageUrl;
        const step1b = await request({ hostname: loginPageHost, port: loginPagePort, path: loginPath, method: 'GET', headers: baseHeaders() });
        jar.ingest(step1b.headers);
        info(`[tokenManager] Step 1b: HTTP ${step1b.statusCode}`);
        const matchCsk = step1b.body.match(/name=["']connector_session_key["'][^>]*value=["']([^"']+)["']|value=["']([^"']+)["'][^>]*name=["']connector_session_key["']/);
        const matchCsrf = step1b.body.match(/name=["']_csrf["'][^>]*value=["']([^"']+)["']|value=["']([^"']+)["'][^>]*name=["']_csrf["']/);
        connectorSessionKey = (matchCsk && (matchCsk[1] || matchCsk[2])) || '';
        csrfToken = (matchCsrf && (matchCsrf[1] || matchCsrf[2])) || '';
        info(`[tokenManager] Step 1b: connector_session_key=${connectorSessionKey ? connectorSessionKey.slice(0, 8) + '…' : 'not found'}, _csrf=${csrfToken ? csrfToken.slice(0, 8) + '…' : 'not found'}`);
        info(`[tokenManager] Step 1b: body preview=${step1b.body.slice(0, 400).replace(/\s+/g, ' ')}`);
    } else {
        info(`[tokenManager] Step 1b: no redirect location, skipping login page fetch`);
    }
