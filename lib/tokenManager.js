...
throw new Error(`Unknown brand: ${brand}`);
    ...
    if (loginPageUrl) {
        ...
        const loginPath = loginPageUrl.startsWith('http') ? new URL(loginPageUrl).pathname + new URL(loginPageUrl).search : loginPageUrl;
        const loginPageHost = loginPageUrl.startsWith('http') ? new URL(loginPageUrl).hostname : host;
        const step1b = await request({ hostname: loginPageHost, path: loginPath, method: 'GET', headers: baseHeaders() });
