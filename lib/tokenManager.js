...
throw new Error(`Unknown brand: ${brand}`);
    ...
    // Step 1b: visit CCAPI page so the connector session gets registered in the backend
    if (step1Location) {
        try {
            const ccapiParsed = new URL(step1Location);
            const ccapiPort = parseInt(ccapiParsed.port || '443');
            info(`[tokenManager] Step 1b: GET ${ccapiParsed.hostname}:${ccapiPort}${ccapiParsed.pathname.slice(0, 40)}`);
            const step1b = await request({
                hostname: ccapiParsed.hostname,
                port: ccapiPort,
                path: ccapiParsed.pathname + ccapiParsed.search,
                method: 'GET',
                headers: baseHeaders(),
            });
            jar.ingest(step1b.headers);
            info(`[tokenManager] Step 1b: HTTP ${step1b.statusCode}, cookies: ${Object.keys(jar._cookies).join(', ')}`);
        } catch (e) {
            info(`[tokenManager] Step 1b: ${e.message}`);
        }
    }
