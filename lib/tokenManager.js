...
/**
 * Node.js-only OAuth flow (no TLS impersonation).
 *
 * @param {string} brand
 ...
 * @param {Function} [log]
 */
async function _fetchTokenNodeJsInner(brand, username, password, log) {
    const info = log || (() => {});
...
throw new Error(`Unknown brand: ${brand}`);
    ...
    // Step 1b: save _hazkpw before any further requests — it is the IDP CSRF token
    const hazkpwFromStep1 = jar._cookies['_hazkpw'] || '';
    info(`[tokenManager] Step 1b: _hazkpw=${hazkpwFromStep1 ? `${hazkpwFromStep1.slice(0, 8)  }…` : 'not found'}`);
...
throw new Error(`No JWK in certs response: ${step2.body.slice(0, 200)}`);
    ...
    // Step 3: POST signin
    info(`[tokenManager] Step 3: POST https://${host}/auth/account/signin (csrf=${hazkpwFromStep1 ? `${hazkpwFromStep1.slice(0, 8)  }…` : 'empty'})`);
...
/**
 * Fetch token via Python subprocess (curl_cffi Chrome TLS impersonation).
 * Stdout: JSON result. Stderr: diagnostic log lines forwarded via `log`.
 *
 * @param {string} brand
 * @param {string} username
 * @param {string} password
 * @param {Function} [log]
 */
function fetchTokenViaPython(brand, username, password, log) {
    const info = log || (() => {});
    const script = path.join(__dirname, 'fetchtoken.py');
    const arg = JSON.stringify({ brand, username, password });

    return new Promise((resolve, reject) => {
        execFile('python3', [script, arg], { timeout: 30000 }, (err, stdout, stderr) => {
            if (stderr) {
                stderr.trim().split('\n').forEach(l => info(`[tokenManager] ${l}`));
            }
            if (err && !stdout) {
                return reject(new Error(`python3 script failed: ${err.message}`));
            }
            let result;
            try {
                result = JSON.parse(stdout);
            } catch (_) {
                return reject(new Error(`Invalid JSON from python3: ${stdout.slice(0, 100)}`));
            }
            if (result.error) {
...
/**
 * Fetch a new refresh token — tries Python (curl_cffi) first, falls back to Node.js.
 *
...
async function fetchToken(brand, username, password, log) {
    const info = log || (() => {});
    try {
        info('[tokenManager] Trying Python/curl_cffi path …');
        const result = await fetchTokenViaPython(brand, username, password, info);
        info('[tokenManager] Python path succeeded');
        return result;
    } catch (pyErr) {
        info(`[tokenManager] Python path failed (${pyErr.message}), falling back to Node.js`);
    }
    return fetchTokenNodeJs(brand, username, password, log);
}

/**
 * Pure Node.js OAuth flow (fallback — no TLS impersonation).
 *
 * @param {string} brand
 * @param {string} username
 * @param {string} password
 * @param {Function} [log]
 */
async function fetchTokenNodeJs(brand, username, password, log) {
    return _fetchTokenNodeJsInner(brand, username, password, log);
}
