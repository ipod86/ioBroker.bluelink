...
throw new Error(`Unknown brand: ${brand}`);
    ...
    // Step 1b: Call CCAPI API authorize → redirects to IDP login page with connector_session_key in URL
    let connectorSessionKey = '';
    let csrfToken = '';
    const ccapiBase = new URL(cfg.redirectUri);
    const ccapiHostname = ccapiBase.hostname;
    const ccapiPort = parseInt(ccapiBase.port || 443);
    const ccapiAuthPath = `/api/v1/user/oauth2/authorize?response_type=code&client_id=${cfg.clientId}&redirect_uri=${encodeURIComponent(cfg.redirectUri)}&lang=de&state=ccsp&country=DE`;
    const basicAuth = Buffer.from(`${cfg.clientId}:${cfg.clientSecret}`).toString('base64');
    info(`[tokenManager] Step 1b: GET https://${ccapiHostname}:${ccapiPort}${ccapiAuthPath.slice(0, 60)}`);
    const step1b = await request({ hostname: ccapiHostname, port: ccapiPort, path: ccapiAuthPath, method: 'GET', headers: baseHeaders({
        'Authorization': `Basic ${basicAuth}`,
        'ccsp-service-id': cfg.clientId,
        'Accept': 'application/json, text/plain, */*',
    }) });
    jar.ingest(step1b.headers);
    info(`[tokenManager] Step 1b: HTTP ${step1b.statusCode}`);
    const idpLoginUrl = step1b.headers['location'] || '';
    info(`[tokenManager] Step 1b: location=${idpLoginUrl.slice(0, 120)}`);
    if (idpLoginUrl) {
        try {
            const idpLoginParsed = new URL(idpLoginUrl.startsWith('http') ? idpLoginUrl : `https://${host}${idpLoginUrl}`);
            connectorSessionKey = idpLoginParsed.searchParams.get('connector_session_key') || '';
        } catch (_) { /* ignore parse error */ }
    }
    if (!connectorSessionKey) {
        // Fallback: follow to IDP login page and parse hidden field from HTML
        const loginTarget = idpLoginUrl || step1.headers['location'] || '';
        if (loginTarget) {
            const lp = loginTarget.startsWith('http') ? new URL(loginTarget) : null;
            if (lp) {
                const step1c = await request({ hostname: lp.hostname, port: parseInt(lp.port || 443), path: lp.pathname + lp.search, method: 'GET', headers: baseHeaders() });
                jar.ingest(step1c.headers);
                info(`[tokenManager] Step 1c: HTTP ${step1c.statusCode}`);
                const m = step1c.body.match(/name=["']connector_session_key["'][^>]*value=["']([^"']+)["']|value=["']([^"']+)["'][^>]*name=["']connector_session_key["']/);
                const mc = step1c.body.match(/name=["']_csrf["'][^>]*value=["']([^"']+)["']|value=["']([^"']+)["'][^>]*name=["']_csrf["']/);
                connectorSessionKey = (m && (m[1] || m[2])) || '';
                csrfToken = (mc && (mc[1] || mc[2])) || '';
            }
        }
    }
    info(`[tokenManager] Step 1b: connector_session_key=${connectorSessionKey ? connectorSessionKey.slice(0, 8) + '…' : 'not found'}`);
...
throw new Error(`No JWK in certs response: ${step2.body.slice(0, 200)}`);
    ...
    // Step 3: POST signin
    // _hazkpw cookie is the IDP's CSRF token (double-submit pattern)
    const hazkpw = jar._cookies['_hazkpw'] || '';
    const effectiveCsrf = csrfToken || hazkpw;
    info(`[tokenManager] Step 3: POST https://${host}/auth/account/signin (csrf=${effectiveCsrf ? effectiveCsrf.slice(0, 8) + '…' : 'empty'})`);
