 ...
 */
async function fetchToken(brand, username, password) {
    ...
    if (step2.statusCode !== 200) throw new Error(`Certs endpoint returned ${step2.statusCode}`);
    const jwk = JSON.parse(step2.body).retValue;
    if (!jwk || !jwk.kid) throw new Error('No JWK in certs response');
